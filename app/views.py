...
def ins(request):
    if request.method == 'POST':
        ...
        einrichtung = request.POST.get('einrchtng','')
        email = request.POST.get('email','')
        phone = request.POST.get('tel','')
        finanzstelle = request.POST.get('finanz','')
        GültigVon = request.POST.get('von','')
        GültigBis = request.POST.get('bis','')
        savedata = grunddaten(sapname=sapname, vorname=vorname, nachname=nachname, instno=instno, einrichtung=einrichtung, email=email, phone=phone, finanzstelle=finanzstelle,GültigVon=GültigVon, GültigBis=GültigBis)
        savedata.save()
        return HttpResponse("Inserted Data into Antrag Database..")
    else:
